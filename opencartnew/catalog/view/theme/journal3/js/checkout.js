$(function () {
	window['_QuickCheckout'] = new Vue({
		...
		mounted: function () {
			...
			if (phoneInputField) {
				self.iti = intlTelInput(phoneInputField, {
			...
			var loginPhoneInputField = document.querySelector("#input-login-telephone");
			if (loginPhoneInputField) {
				self.login_iti = intlTelInput(loginPhoneInputField, {
					preferredCountries: ["my","sg","id","th","vn","ph","bn","mm"],
					initialCountry: "my",
					separateDialCode: true,
					utilsScript:
			...
			console.log('loginPhoneInputField', loginPhoneInputField);
		...
		methods: {
			...
			login: function () {
				var phoneInput = $('#input-login-telephone').val();
				var countryData = this.login_iti.getSelectedCountryData();
				...
				if (phoneNumber && phoneNumber.isValid()) {
					...
					this.login_telephone = telephone;
				} else {              
					this.login_telephone = phoneInput;
				}

				var data = {
					telephone: this.login_telephone,
			...
			verify: function() {
				...
				console.log(self)
				var otpInput = $('#input-otp-code').val();
				...
				if (phoneNumber && phoneNumber.isValid()) {
					...
					// $('#telephone').val(phone);
					this.order_data.telephone = phone;
					...
					$.ajax({
						...
						data: {phone: phone, otp_forgotten: 0},
						success: function(json){
							$('#error_code').hide();
						  
							if (json.success) {
								self.otp_code = otpInput;
					...
					$('.account-telephone').addClass('has-error');
					$('#phone_message').addClass('text-danger').html('Phone number empty or invalid');
					$('#error_code').hide();
				}
			},
			startOtpCountdown: function() {
				...
				self.counter = parseInt(self.otp_resend)
				self.resend_interval = setInterval(function() {
					self.counter--;
					if (self.counter <= 0) {
						self.stopOtpCountdown()
					} else {
						var text = 'Resend (%s)';
						$('#btn-verify-otp').text(text.replace(/%s/g, self.counter));
						$('#btn-verify-otp').prop('disabled', true);
						self.countdowning = true
					}
				}, 1000);
			},
			stopOtpCountdown: function() {
				var self = this;

				if (self.resend_interval) {
					clearInterval(self.resend_interval);
				}
				$('#btn-verify-otp').text('Resend');
